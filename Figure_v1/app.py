...
def sec2A():
    return html.Div(children=[
        ...
        html.Label('Truncate by Raman Shift values: '),
        dcc.Input(id='truncate_before'),
        dcc.Input(id='truncate_after'),
        ...
        dcc.Slider(-1,1.5,value=1.1,id='det-ay',tooltip=slider_layout)
...
def closet_value(numbers_list,search_value):
    return min(numbers_list, key=lambda x: abs(x - search_value))

def find_index(df,value,col=0):
    thecol=df.iloc[:,col]
    num=closet_value(thecol.tolist(),value)
    return df.index[df.iloc[:,col] == num][0]
...
@app.callback(
    ...
    Input('details','value'),
    Input('peaks','value'),
    Input('truncate_before','value'),Input('truncate_after','value')]
    )
...
def update_output(list_of_contents,list_of_filenames, #Input from Files
                  ...
                  title,details,peaks, #Text inputs
                  rs_start,rs_end
                  ):
    ...
    if list_of_filenames is not None:
        ...
        for content_string, name in zip(list_of_contents,list_of_filenames):
            ...
            if rs_start is None:
                pass
            elif rs_start is not None:
                in_b=(find_index(decoded,int(rs_start)))
                decoded=decoded.truncate(before=in_b)
            if rs_end is None:
                pass
        ...
        fig.update_xaxes(title_font_size=80)
